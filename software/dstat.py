...
class dstat_time(dstat):
    def __init__(self):
        self.name = 'epoch_ns'  # Rename the plugin for clarity
        self.type = 's'         # Keep output as string (for compatibility)
        self.width = 19         # Length of a 64-bit nanosecond timestamp
        self.scale = 0
        self.vars = ('time',)

    def extract(self):
        # Calculate timestamp in nanoseconds since epoch
        timestamp_ns = int(starttime * 1e9)
        self.val['time'] = str(timestamp_ns)
