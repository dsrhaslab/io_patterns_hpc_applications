...
def evaluate_and_plot(true_values, predictions, output_folder, debug):
...
def main(debug=True):
    ...
    # Preprocess train
    train_df = preprocessing_aggregate(raw_train_data, TIME_THRESHOLD, burst_threshold, debug_base, is_train=True)

    save_decoded_matrix(train_df, output_folder, "train")
    ...
    time_windows = [i * TIME_THRESHOLD for i in range(len(all_predictions))]
    df = pd.DataFrame({
        "time_window": time_windows,
        "seconds_to_next_burst": all_predictions
    })
    save_decoded_matrix(df, output_folder, "predictions")
    df_true = pd.DataFrame({
        "time_window": time_windows,
        "seconds_to_next_burst": true_values
    })
    save_decoded_matrix(df_true, output_folder, "true_values")

    # Evaluate
    evaluate_burst(true_values, all_predictions, output_folder, debug)
