...
SCRIPT="$HOME/PyTorch/bert-from-scratch/bert-from-scratch.py"
...
if [[ ! -f "$MOVIE_CONVERSATIONS" || ! -f "$MOVIE_LINES" ]]; then
    ...
    mv "cornell movie-dialogs corpus/movie_conversations.txt" "$MOVIE_CONVERSATIONS"
    mv "cornell movie-dialogs corpus/movie_lines.txt" "$MOVIE_LINES"
...
torchrun --nnodes 4 \
	...
	--rdzv_id $RANDOM \
	--rdzv_backend c10d \
	--rdzv_endpoint $MASTER_ADDR:$MASTER_PORT $SCRIPT \
		--epochs 30 \
		--batchsize 128 \
		--maxlen 64 \
		--dataset "$DATASET_DIR" \
